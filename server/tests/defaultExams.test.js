...
test("every catalogue logo exists in local project assets", async () => {
  ...
  await Promise.all(
    defaultExams.map(({ logo }) =>
      access(path.resolve(here, "../../client/public", logo.slice(1))),
    ),
  );
});

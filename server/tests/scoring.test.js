...
function assertAlignment(source, typed, expected) {
  ...
  assert.deepEqual(result, {
    referenceCharacters: Array.from(source.normalize("NFC")).length,
    typedCharacters: Array.from(typed.normalize("NFC")).length,
    ...expected,
  });
  assert.equal(
    result.correctCharacters +
      result.wrongCharacters +
      result.omittedCharacters,
    result.referenceCharacters,
  );
  assert.equal(
    result.correctCharacters + result.wrongCharacters + result.extraCharacters,
    result.typedCharacters,
  );
  assert.equal(
    result.wrongCharacters + result.omittedCharacters + result.extraCharacters,
    result.totalErrors,
  );
}

test("exact text has full accuracy and five-keystroke gross WPM", () => {
  const result = calculateResult("hello world", "hello world", 60, {
    totalKeystrokes: 13,
    backspaceCount: 2,
  });
...
test("distinguishes substitution, middle omission and middle insertion without cascading", () => {
  ...
  assertAlignment("abcdef", "abcXdef", {
    correctCharacters: 6,
    wrongCharacters: 0,
    omittedCharacters: 0,
    extraCharacters: 1,
...
test("normalizes Unicode and handles Hindi and mixed-language text", () => {
  ...
  assertAlignment("भारत SSC", "भारत SC", {
    correctCharacters: 7,
    wrongCharacters: 0,
    omittedCharacters: 1,
    extraCharacters: 0,
  ...
  const hindiMistake = calculateResult(
    "भारत एक देश है",
    "भारत एक देस है",
    60,
    {},
    { mode: "standard-word", errorPenalty: 1 },
  );
  const englishMistake = calculateResult(
    "India is a country",
...
test("formulas remain internally consistent with errors and early submission", () => {
  const oneError = calculateResult(
...
test("word alignment identifies wrong, omitted and extra words independently", () => {
  assert.deepEqual(alignWords("one two three", "one too three"), {
    typedWords: 3,
    referenceWords: 3,
    wrongWords: 1,
    omittedWords: 0,
  ...
  assert.deepEqual(alignWords("one two three", "one three"), {
    typedWords: 2,
    referenceWords: 3,
    wrongWords: 0,
    omittedWords: 1,
  ...
  assert.deepEqual(alignWords("one two three", "one extra two three"), {
    typedWords: 4,
    referenceWords: 3,
    wrongWords: 0,
    omittedWords: 0,
...
test("the classified error total controls Net WPM independently of legacy display mode", () => {
  ...
  const typed = "one too three";
  const wordMode = calculateResult(
    source,
    typed,
    60,
    {},
  ...
  const characterMode = calculateResult(
    source,
    typed,
    60,
    {},
    { mode: "character", errorPenalty: 1 },
  );
  const doublePenalty = calculateResult(
    source,
    typed,
    60,
    {},
    { mode: "standard-word", errorPenalty: 2 },
  );
  ...
  for (const result of [wordMode, characterMode, doublePenalty])
    assert.ok(result.netWpm <= result.grossWpm);
});
...
test("result formulas cover perfect, many-error, no-input, short and long passages", () => {
  ...
  for (const [name, source, typed, seconds] of cases) {
    ...
    const expectedGross =
      Math.round((Array.from(typed).length / 5 / (seconds / 60)) * 100) / 100;
    const expectedAccuracy = result.referenceCharacters
      ? Math.round(
          (Math.max(0, result.referenceCharacters - result.weightedErrors) /
            result.referenceCharacters) *
            100 *
            100,
        ) / 100
...
test("non-steno mode classifies half errors and exposes the exact highlighted spans", () => {
  ...
  assert.equal(
    capitalization.typedParts.find((part) => part.severity !== "correct")
      .category,
    "capitalization",
  );
  ...
  assert.equal(
    spacing.comparison.typedParts.some((part) => part.severity === "half"),
    true,
  );
});
...
test("Steno mode promotes every half-category mistake to a full error", () => {
  ...
  const ssc = calculateResult(
    "Hello, world",
    "hello world",
    60,
    {},
    { evaluationMode: "ssc-stenographer" },
  );
  ...
  assert.equal(
    ssc.comparison.typedParts
      .filter((part) => part.severity !== "correct")
      .every((part) => part.severity === "full"),
    true,
  );
});
...
test("persisted comparison spans and category totals share one source of truth", () => {
  ...
  const categoryTotal = Object.values(result.errorBreakdown).reduce(
    (sum, value) => sum + value,
    0,
  );
  assert.equal(categoryTotal, result.fullErrors + result.halfErrors);
  assert.equal(
    result.weightedErrors,
    result.fullErrors + result.halfErrors * 0.5,
  );
  assert.equal(
    result.comparison.referenceParts.map((part) => part.text).join(""),
    "Hello, one two",
  );
  assert.equal(
    result.comparison.typedParts.map((part) => part.text).join(""),
    "hello one  too",
  );
});

test("word boundaries prevent capitalization from aligning with a future word", () => {
  const source = "responsibility. A candidate";
  const typed = "responsibility. a candidate";
  const result = classifyErrors(source, typed);
  assert.equal(result.counts.capitalization, 1);
  assert.equal(result.halfErrors, 1);
  assert.equal(result.fullErrors, 0);
  assert.deepEqual(
    result.referenceParts.filter((part) => part.severity !== "correct"),
    [{ text: "A", severity: "half", category: "capitalization" }],
  );
  assert.deepEqual(
    result.typedParts.filter((part) => part.severity !== "correct"),
    [{ text: "a", severity: "half", category: "capitalization" }],
  );
...
test("a full error inside a paired word highlights both entire words", () => {
  ...
  assert.deepEqual(result.referenceParts, [
    { text: "strong", severity: "full", category: "incompleteWord" },
  ]);
  assert.deepEqual(result.typedParts, [
    { text: "strog", severity: "full", category: "incompleteWord" },
  ]);
});

test("word alignment recovers after long omissions and repeated identical words", () => {
  const omitted = classifyErrors(
    "start one two three four five six finish",
    "start finish",
  );
  assert.equal(omitted.counts.omission, 6);
  assert.equal(
    omitted.referenceParts.map((part) => part.text).join(""),
    "start one two three four five six finish",
  );
  assert.equal(
    omitted.typedParts.map((part) => part.text).join(""),
    "start finish",
  );
  assert.deepEqual(alignWords("go go stop go", "go go go stop go"), {
    typedWords: 5,
    referenceWords: 4,
    wrongWords: 0,
    omittedWords: 0,
  ...
  assert.deepEqual(
    repeated.typedParts.filter((part) => part.severity !== "correct"),
    [{ text: "go", severity: "full", category: "repetition" }],
  );
});

test("comparison projections contain only real source and typed text", () => {
  const source = "one\ntwo strong";
  const typed = "one two strog";
  const result = classifyErrors(source, typed);
  assert.equal(result.referenceParts.map((part) => part.text).join(""), source);
  assert.equal(result.typedParts.map((part) => part.text).join(""), typed);
  for (const part of [...result.referenceParts, ...result.typedParts])
...
test("adaptive alignment distance matches an independent reference algorithm", () => {
  ...
  for (let run = 0; run < 100; run += 1) {
    ...
    const typed = make();
    assert.equal(
      alignCharacters(source, typed).totalErrors,
      referenceDistance(source, typed),
    );
  }
});
...
test("backspace and highlight preference combinations cannot alter scoring rules", () => {
  for (const backspaceEnabled of [true, false]) {
    for (const wordHighlight of [true, false]) {
      ...
      const telemetry = backspaceEnabled
        ? { totalKeystrokes: 14, backspaceCount: 1 }
        : { totalKeystrokes: 12, backspaceCount: 0 };
      const result = calculateResult("correct text", finalText, 60, telemetry);
      assert.equal(
        result.totalErrors,
        backspaceEnabled ? 0 : 1,
        `backspace=${backspaceEnabled}, highlight=${wordHighlight}`,
      );
      assert.equal(result.backspaceCount, backspaceEnabled ? 1 : 0);
      assert.equal(result.correctCharacters, backspaceEnabled ? 12 : 11);
      assert.equal(
        result.netWpm,
        backspaceEnabled ? result.grossWpm : Math.max(0, result.grossWpm - 1),
      );
    }
  }
});

...
const sampleParagraph = async (exam) => {
  const [paragraph] = await Paragraph.aggregate([
    { $match: { exam: exam._id, language: exam.language } },
    { $sample: { size: 1 } },
    { $project: { title: 1, content: 1, language: 1, difficulty: 1 } },
  ]);
  if (!paragraph)
    throw new AppError("No paragraph is available for this exam", 404);
  return paragraph;
};
...
export const createParagraph = asyncHandler(async (req, res) => {
  ...
  if (req.body.language !== exam.language)
    throw new AppError(
      `Paragraph language must match the exam language (${exam.language})`,
      400,
    );
  ...
  res
    ...
    .json({
      success: true,
      paragraph: await paragraph.populate("exam", "name"),
    });
});
export const updateParagraph = asyncHandler(async (req, res) => {
  ...
  const paragraph = await Paragraph.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true,
  }).populate("exam", "name");
...
export const deleteParagraph = asyncHandler(async (req, res) => {
  ...
  if (resultCount)
    throw new AppError(
      `This paragraph has ${resultCount} saved result${resultCount === 1 ? "" : "s"} and cannot be deleted.`,
      409,
    );
...
export const listExamParagraphs = asyncHandler(async (req, res) => {
  const exam = await Exam.findOne({ _id: req.params.id, status: "active" })
    .select(
      "name organization language category durationMinutes logo description",
    )
    .lean();
  if (!exam) throw new AppError("Exam is unavailable", 404);
  const paragraphs = await Paragraph.aggregate([
    ...
    { $sort: { difficultyOrder: 1, createdAt: -1, title: 1 } },
    {
      $project: {
        ...
        difficulty: 1,
        wordCount: {
          $size: { $regexFindAll: { input: "$content", regex: /\S+/ } },
        },
      },
    },
  ]);
...
export const launchTest = asyncHandler(async (req, res) => {
  ...
  const paragraph = await Paragraph.findOne({
    _id: req.body.paragraphId,
    exam: exam._id,
    language: exam.language,
  })
    .select("title content language difficulty")
    .lean();
  if (!paragraph)
    throw new AppError("Paragraph is unavailable for this exam", 404);
  if (exam.category === "Practice")
    return res.json({ success: true, requiresSettings: true, exam, paragraph });

...
export const env = {
  ...
  jwtSecret:
    process.env.JWT_SECRET || "development-only-secret-change-me-please",
...
if (
  ...
  /127\.0\.0\.1|localhost/.test(env.mongoUri)
) {
...
if (env.nodeEnv === "production") {
  ...
  if (
    ...
    throw new Error(
      "CLIENT_URL must contain the HTTPS origin of the production frontend",
    );
  }
  if (clientOrigins.some((origin) => origin.includes("*"))) {
    throw new Error(
      "CLIENT_URL must use exact origins; wildcards are not allowed in production",
    );
  }
}

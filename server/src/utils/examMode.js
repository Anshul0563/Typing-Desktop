...
export function resolveExamMode(exam, requestedMode) {
  ...
  return Object.values(EXAM_MODES).includes(requestedMode)
    ? requestedMode
    : EXAM_MODES.CUSTOM;
}
...
export function resolveDurationSeconds(exam, requestedMinutes) {
  ...
  if (!isPracticeExam(exam) || requestedMinutes == null)
    return configuredMinutes * 60;
  const minutes = Math.min(
    120,
    Math.max(1, Number(requestedMinutes) || configuredMinutes),
  );
  return Math.round(minutes * 60);
}

export function resolveEvaluationMode(exam) {
  return /\bsteno(?:grapher)?\b/i.test(String(exam?.name || ""))
    ? "ssc-stenographer"
    : "practice";
}

...
export async function ensureDefaultCatalogue({ logger = console } = {}) {
  const catalogueWrite = await Exam.bulkWrite(
    defaultExams.map((exam) => ({
      updateOne: {
        filter: { name: exam.name },
  ...
  const existingParagraphs = await Paragraph.find({
    exam: { $in: exams.map((exam) => exam._id) },
  })
    .select("exam language")
    .lean();
  const coveredExams = new Set(
    existingParagraphs.map(
      (paragraph) => `${paragraph.exam}:${paragraph.language}`,
    ),
  );
  const paragraphs = exams
    .filter((exam) => !coveredExams.has(`${exam._id}:${exam.language}`))
    .map(starterParagraphFor);
  if (paragraphs.length)
    await Paragraph.insertMany(paragraphs, { ordered: false });

  const created = catalogueWrite.upsertedCount || 0;
  logger.info?.(
    `Default catalogue ensured: ${created} exams and ${paragraphs.length} sample paragraphs created.`,
  );

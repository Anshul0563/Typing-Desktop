...
export default function ManageUsers() {
  ...
  return <><div className="admin-page-heading"><div><span>Learners</span><h1>Manage users</h1><p>Search accounts and control access securely.</p></div><div className="admin-heading-badge"><Users />{users.length} shown</div></div><Notice>{error}</Notice><div className="admin-toolbar"><label className="admin-search"><Search /><input placeholder="Search by name or email…" value={search} onChange={(e) => setSearch(e.target.value)} /></label></div>{loading ? <Loader label="Loading users…" /> : users.length ? <section className="admin-user-list"><div className="admin-user-list-head" aria-hidden="true"><span /><span>User details</span><span>Access & actions</span></div><div className="admin-user-grid">{users.map((user) => <article key={user._id}><div className="admin-user-avatar">{user.name.slice(0, 1).toUpperCase()}</div><div className="admin-user-info"><strong>{user.name}</strong><span>{user.email}</span><small>Joined {new Date(user.createdAt).toLocaleDateString()}</small></div><div className="admin-user-control"><span className={`badge badge-${user.isActive ? 'active' : 'inactive'}`}>{user.isActive ? 'Active' : 'Blocked'}</span><Button variant="secondary" disabled={updating === user._id} onClick={() => toggle(user._id)}>{user.isActive ? <><UserRound />Block</> : <><ShieldCheck />Restore</>}</Button></div></article>)}</div></section> : <div className="admin-empty"><Users /><h2>No users found</h2><p>Try a different name or email address.</p></div>}</>;
}
